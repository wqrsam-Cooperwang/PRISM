...
def test_first_legacy_airtable_cohort_is_replayable_and_deterministic() -> None:
    ...
    assert result.summary.v1_primary_hits == 1
    assert result.summary.v21_primary_hits == 0
    assert result.summary.v1_dual_hits == 1
    assert result.summary.v21_dual_hits == 0
    assert result.summary.v1_mean_minimum_distance == pytest.approx(1.25)
    assert result.summary.v21_mean_minimum_distance == pytest.approx(1.25)
    ...
    assert result.summary.distance_tied_cases == 6
