"""Manual CLI for one live TheSportsDB event smoke test."""
...
from src.acquisition.the_sports_db_smoke import run_live_thesportsdb_smoke
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run one live PRISM TheSportsDB smoke test")
    parser.add_argument("--home", required=True, help="Home team name")
    parser.add_argument("--away", required=True, help="Away team name")
    parser.add_argument("--competition", required=True, help="Competition label")
    parser.add_argument("--season", required=True, help="Season label")
...
def main() -> int:
    ...
    run_id = uuid4().hex
    target = MatchTarget(
        match_id=f"live-thesportsdb-{run_id}",
    ...
    request = ProviderFetchRequest(
        request_id=f"live-thesportsdb-{run_id}",
        target=target,
        requested_at=datetime.now(timezone.utc),
    )
    client = TheSportsDbEventSearchClient(transport=StdlibHttpTransport())
    summary = run_live_thesportsdb_smoke(request, client)
