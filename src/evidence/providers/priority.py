...
class PriorityEngine:
    ...
    def produce_evidence(self, match_context: Mapping[str, Any]) -> list[EvidenceResult]:
        ...
        # fixture priority similar but more sensitive to days_to_next
        fixture_raw = 1.2 * (1.0 - min(1.0, days_to_next / 4.0)) + 0.2 * opponent_strength
        fixture_priority = 1.0 / (1.0 + math.exp(-fixture_raw))

        # expected rotation probability inferred inversely from priority and squad depth
        ...
        evidence = EvidenceResult(
            ...
            suggestion={"cup_priority": cup_priority, "fixture_priority": fixture_priority, "rotation_probability": p_rot},
            variance={"cup_priority": 0.02, "fixture_priority": 0.03, "rotation_probability": p_rot * (1 - p_rot)},
            reliability=0.75,
            weight=1.0,
            direction={"cup_priority": cup_priority - 0.5, "fixture_priority": fixture_priority - 0.5, "rotation_probability": p_rot - 0.1},
            confidence=0.75,
